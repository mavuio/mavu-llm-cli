...
When creating new resources, **always** look for existing patterns in the reference codebase at `/www/mavu_codesamples/samples/` first. Search for similar implementations, naming conventions, and structural patterns before writing new code from scratch. Stick as closely as possible to the style and structure found there.
...
use DaisyUI for all UI components, prefer them to building your own.
dont user eplicit colors, use the base-x ones by default to enable dark mode switching.
